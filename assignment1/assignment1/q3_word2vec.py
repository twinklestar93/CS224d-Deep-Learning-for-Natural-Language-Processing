...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    delta = probabilities
    delta[target] -= 1
    N = delta.shape[0]  # The number of all tokens
    D = predicted.shape[0]
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    selVecs = outputVectors[indices, :]
    tmp = sigmoid(selVecs.dot(predicted) * labels)
    cost = -np.sum(np.log(tmp))
    
    delta = labels * (tmp - 1)
    gradPred = delta.reshape((1,K+1)).dot(selVecs).flatten()
